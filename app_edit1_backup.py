...
#conn = psycopg2.connect("host=hbcdm.cxxx3jvy2l3m.us-east-1.rds.amazonaws.com dbname=hbcdm user=hbadmin password=hbaccess")
conn = psycopg2.connect("host= hbcdm.cznh5reqtqiu.us-east-1.rds.amazonaws.com dbname=hbcdm user=hbadmin password=hbaccess")
...
def dashboard():
   ...
   # cur.execute('SELECT * FROM data_catalog where "dataset_name" = %s', [datasetname])
    ...
    result_get = requests.get("http://72.36.65.66:3000/api/Dataset", params=get_data_params)
...
def submithipaaform(requestid_domain,dataset_id_final,dataset_risk):
     ...
     user = requests.get("http://72.36.65.66:3000//api/User", params=get_user_params)
     data = requests.get("http://72.36.65.66:3000/api/Dataset", params=get_data_params)
     ...
     if user.status_code == '200' or user.content != '[]':
         r_post_user = requests.post("http://72.36.65.66:3000/api/User", json=user_json)
     print(user.content)
     if data.status_code == '200' or data.content != '[]':
         data = requests.post("http://72.36.65.66:3000/api/Dataset", json=dataset_json)
     print(data.content)
    # trans = '{"request_id": %s, "inputcs": %s, "inputdr": %s, "dataset": %s, "user": %s}' % (requestid_domain,compliance, dr, data.content , "resources:org.honestchain.User#" +str(current_user.id))
     ...
     trans_post = requests.post("http://72.36.65.66:3000/api/ChainTransaction", json=trans_json)
