...
class TrustCalcForm(UserMixin, db.Model):
    ...
    Do_you_need_radiology_images = db.Column(db.String(30))
...
class CreateTrustCalcForm(FlaskForm):
    #CaStatus = QuerySelectField('Enter your choice', choices=[('Yes', 'Yes'), ('No', 'No'), ('Uncertain', 'Uncertain')])i
     ...
     Do_you_need_radiology_images= QuerySelectField(query_factory=choice_trustcalc, allow_blank=True, get_label = 'decision')
...
def submithipaaform(requestid_domain,dataset_id_final,dataset_risk):
     ...
     Do_you_need_radiology_images = form.Do_you_need_radiology_images.data.decision
     ...
     templist = [Do_you_need_radiology_images, radiology_imaging_reports, ekg, progress_notes, history_phy, oper_report, path_report, lab_report, photographs, discharge_summaries, health_care_billing, consult, medication, emergency, dental, demographic, question, audiotape]
    ...
    # pendingreq_info.requestid=requestId 
     ...
     new_hipaa_request = TrustCalcForm(ownerid =  current_user.id, requestid = requestid_domain, Do_you_need_radiology_images = Do_you_need_radiology_images, radiology_imaging_reports = radiology_imaging_reports, ekg = ekg, progress_notes = progress_notes, history_phy = history_phy, oper_report = oper_report, path_report = path_report, lab_report = lab_report, photographs = photographs, discharge_summaries = discharge_summaries,  health_care_billing= health_care_billing, consult = consult, medication = medication, emergency = emergency, dental  = dental, demographic = demographic,question = question, audiotape = audiotape, compliance=compliance, status = status)
